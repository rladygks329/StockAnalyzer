...
class StockDataCollector:
    ...
    def get_fundamental_data(self, date: str, tickers: list[str]) -> dict:
        ...
        # 전체 PER/PBR/EPS/DIV 데이터
        try:
            fundamental_kospi = pykrx_stock.get_market_fundamental(
                date, market="KOSPI"
            )
            fundamental_kosdaq = pykrx_stock.get_market_fundamental(
                date, market="KOSDAQ"
            )
            fundamental_all = pd.concat([fundamental_kospi, fundamental_kosdaq])
        except Exception as e:
            logger.error(f"[재무지표] 전체 재무지표 수집 실패: {e}")
        ...
        for ticker in tickers:
            try:
                ...
                if not fundamental_all.empty and ticker in fundamental_all.index:
                ...
                # 업종 PER 평균 (같은 시장 평균으로 대체)
                sector_avg_per = self._get_sector_avg_per(ticker, date, fundamental_all)
                ...
                if eps and prev_eps and prev_eps != 0:
                    eps_change = round(((eps - prev_eps) / abs(prev_eps)) * 100, 1)
                ...
                results.append(
                    {
                        "종목코드": ticker,
                        "종목명": pykrx_stock.get_market_ticker_name(ticker),
                        "PER": None,
                        "PBR": None,
                        "EPS": None,
                        "EPS_전년": None,
    ...
    def _get_prev_eps(self, ticker: str, date: str) -> Optional[float]:
        """전년도 EPS 조회"""
        try:
            fund = pykrx_stock.get_market_fundamental(date, date, ticker)
            if fund.empty:
                return None
            return fund.iloc[0].get("EPS", None)
        except Exception:
            return None
